...
4. **Match the reviewer's Dagger primitive.** When a comment names `@BindsInstance`, `@Qualifier`, `@Provides`, or a scope, use that primitive — don't substitute Kotlin extension functions or `Provider<>` indirection unless the named primitive demonstrably doesn't fit.
