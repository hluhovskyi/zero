...
internal abstract class TypedNavigationArgumentSerializer<Type : Any> : NavigationArgumentSerializer {
    ...
    override fun <T : Any> deserialize(argument: Argument<T>, rawValue: String): ArgumentValue<T> {
        return if (argument.argumentClass == actualClass) {
    ...
    override fun <T : Any> serialize(argumentValue: ArgumentValue<T>): String {
        return if (argumentValue.argument.argumentClass == actualClass) {
    ...
    private fun assertNotSupported(argument: Argument<*>): Nothing {
        throw IllegalStateException("Argument class ${argument.argumentClass} is not supported by this serializer. Supported class: $actualClass")
    }
}