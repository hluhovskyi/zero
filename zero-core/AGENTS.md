...
- **Edit sub-packages** (e.g., `transactions/edit/`): Separate components for create/edit flows. Trap: `TransactionEditMapping.resolve` defaults `selectedCategory` to the first type-matching category, so it's non-null as soon as categories load — a null selection only means "list empty," never "user hasn't chosen yet." Gate any "no choice made yet" UI on the explicit `categoryChosenByUser` flag, not on `selectedCategory == null`.
