...
- [ ] **Step 3: Wire `DefaultSyncComponent` in `ApplicationComponent`**

`ApplicationComponent` is the mediator. It receives `DatabaseComponent` (via its existing graph) and delegates its interface-typed properties into `SyncComponent.Dependencies`. No `Room*` or `Default*` implementation type is referenced here.

```kotlin
// Inside ApplicationComponent:
val syncComponent: SyncComponent by lazy {
    DefaultSyncComponent(
        dependencies = object : SyncComponent.Dependencies {
            ...
            override val transactionSyncSink   = databaseComponent.transactionSinkSource
        }
    )
}

val syncEngine: SyncEngine get() = syncComponent.syncEngine
```
