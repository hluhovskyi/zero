...
**Files:**
- Modify: `app/src/main/java/com/hluhovskyi/zero/ApplicationComponent.kt` (or wherever the app-level component lives)

The wiring follows the manual DI pattern from `docs/agents/di-migration.md`. `DatabaseComponent` provides the sync sources/sinks. `ApplicationComponent` adapts them into `SyncComponent.Dependencies` and constructs `DefaultSyncComponent`.

- [ ] **Step 1: Locate `ApplicationComponent`**

```bash
find app/src -name "ApplicationComponent.kt" -o -name "ActivityComponent.kt" | head -5
```

- [ ] **Step 2: Make `DatabaseComponent` expose sync sources/sinks**

In `DatabaseComponent`, add properties for all six sync source/sink instances. These are the `Room*SyncSource`/`Room*SyncSink` classes from `zero-database`. `DatabaseComponent` already owns the database and `currentUserId` — it has everything needed:

```kotlin
// Inside DatabaseComponent:
val categorySyncSource: EntitySyncSource<SyncCategory> =
...
`ApplicationComponent` (or whichever root component owns `DatabaseComponent`) creates `DefaultSyncComponent` by passing an anonymous `SyncComponent.Dependencies` that delegates to `DatabaseComponent`:

```kotlin
// Inside ApplicationComponent:
val syncComponent: SyncComponent = DefaultSyncComponent(
...
git commit -m "feat(app): wire DefaultSyncComponent via ApplicationComponent → DatabaseComponent"
```
