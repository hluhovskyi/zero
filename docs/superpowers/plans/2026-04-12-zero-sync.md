...
| `zero-sync/src/main/java/com/hluhovskyi/zero/sync/DefaultSyncEngine.kt` | `internal` implementation of `SyncEngine` interface |
| `zero-sync/src/main/java/com/hluhovskyi/zero/sync/SyncComponent.kt` | Wires `DefaultSyncEngine` via `Dependencies` interface |
...
| `zero-api/src/main/java/com/hluhovskyi/zero/sync/SyncEngine.kt` | Engine interface: export, import |
...
- [ ] **Step 8: Create `IdKnownSerializer.kt`**
...
```bash
git add zero-api/
git commit -m "feat(zero-api): add sync interfaces SyncEntity, SyncEngine, EntitySyncSource, EntitySyncSink, ConflictResolver"
```
...
## Task 10: Implement `SyncPipeline`, `DefaultSyncEngine`, and `SyncComponent` (TDD)

**Files:**
- Create: `zero-sync/src/main/java/com/hluhovskyi/zero/sync/SyncPipeline.kt`
- Create: `zero-sync/src/main/java/com/hluhovskyi/zero/sync/DefaultSyncEngine.kt`
- Create: `zero-sync/src/main/java/com/hluhovskyi/zero/sync/SyncComponent.kt`
...
- [ ] **Step 4: Create `DefaultSyncEngine.kt`**

`DefaultSyncEngine` implements `SyncEngine` from `zero-api` and is `internal` (enforced by `DefaultImplMustBeInternal` lint rule). Use a sealed `EntityPipeline` hierarchy to avoid unchecked casts — each subtype handles its own entity type explicitly.
...
internal class DefaultSyncEngine(
    ...
    private suspend fun <T : SyncEntity> mergePipeline(
        ...
        val winners = incoming.flatMap { entity -> pipeline.resolver.resolve(local[entity.id], entity) }
        if (winners.isNotEmpty()) pipeline.localSink.syncUpsert(winners)
    }
}
```

- [ ] **Step 5: Create `SyncComponent.kt`**
...
```bash
git add zero-sync/
git commit -m "feat(zero-sync): implement DefaultSyncEngine and SyncComponent with unit tests"
```
...
## Task 13: Wire `SyncComponent` in `ApplicationComponent`

**Files:**
- Modify: `zero-database/` — `DatabaseComponent` gains interface-typed sync source/sink properties
- Modify: `app/` — `ApplicationComponent` mediates between `DatabaseComponent` and `DefaultSyncComponent`

**Boundary rule:** `RoomXxx` implementation class names must never appear outside `zero-database`. `DatabaseComponent` exposes only `EntitySyncSource<T>` / `EntitySyncSink<T>` interface types. `ApplicationComponent` wires interfaces to interfaces — it imports nothing from `zero-database` except what `DatabaseComponent` already exposes through its public interface. `SyncComponent` stays on manual DI; existing Dagger components are unchanged.

- [ ] **Step 1: Locate `ApplicationComponent` and `DatabaseComponent`**

```bash
find app/src zero-database/src -name "ApplicationComponent.kt" -o -name "DatabaseComponent.kt" | head -5
```

- [ ] **Step 2: Add interface-typed sync properties to `DatabaseComponent`**

Inside `DatabaseComponent`, construct the `Room*` instances internally and expose them only as interface types. No `Room*` type name appears in any public property signature — `ApplicationComponent` can only see `EntitySyncSource<T>` and `EntitySyncSink<T>`.

```kotlin
// Inside DatabaseComponent — interface types on all public properties
val categorySyncSource: EntitySyncSource<SyncCategory> by lazy {
    RoomCategorySyncSource(dao = { database.categorySync() })
}
val categorySyncSink: EntitySyncSink<SyncCategory> by lazy {
    RoomCategorySyncSink(dao = { database.categorySync() }, currentUserId = currentUserId)
}
val accountSyncSource: EntitySyncSource<SyncAccount> by lazy {
    RoomAccountSyncSource(dao = { database.accountSync() })
}
val accountSyncSink: EntitySyncSink<SyncAccount> by lazy {
    RoomAccountSyncSink(dao = { database.accountSync() }, currentUserId = currentUserId)
}
val transactionSyncSource: EntitySyncSource<SyncTransaction> by lazy {
    RoomTransactionSyncSource(dao = { database.transactionSync() })
}
val transactionSyncSink: EntitySyncSink<SyncTransaction> by lazy {
    RoomTransactionSyncSink(dao = { database.transactionSync() }, currentUserId = currentUserId)
}
```

- [ ] **Step 3: Wire `DefaultSyncComponent` in `ApplicationComponent` via Dagger `@Provides`**

`ApplicationComponent` already uses Dagger — keep that. Add a `@Provides` method in the relevant Dagger `@Module` that constructs `DefaultSyncComponent`. Dagger provisions the six interface-typed dependencies automatically from `DatabaseComponent`'s exposed abstract getters. No `by lazy`, no manual construction in the component class itself.

```kotlin
// New SyncModule (or add to existing AppModule):
@Module
object SyncModule {

    @Provides
    @Singleton
    fun provideSyncComponent(
        categorySyncSource: EntitySyncSource<SyncCategory>,
        categorySyncSink: EntitySyncSink<SyncCategory>,
        accountSyncSource: EntitySyncSource<SyncAccount>,
        accountSyncSink: EntitySyncSink<SyncAccount>,
        transactionSyncSource: EntitySyncSource<SyncTransaction>,
        transactionSyncSink: EntitySyncSink<SyncTransaction>,
    ): SyncComponent = DefaultSyncComponent(
        dependencies = object : SyncComponent.Dependencies {
            override val categorySyncSource    = categorySyncSource
            override val categorySyncSink      = categorySyncSink
            override val accountSyncSource     = accountSyncSource
            override val accountSyncSink       = accountSyncSink
            override val transactionSyncSource = transactionSyncSource
            override val transactionSyncSink   = transactionSyncSink
        }
    )

    @Provides
    fun provideSyncEngine(syncComponent: SyncComponent): SyncEngine =
        syncComponent.syncEngine
}
```

`DatabaseComponent` (already a Dagger component dependency of `ApplicationComponent`) must expose each interface-typed property as an abstract getter so Dagger can provision them:

```kotlin
// In DatabaseComponent Dagger interface — abstract getters, interface types only:
...
No `Room*` type appears in `ApplicationComponent`, its modules, or `DatabaseComponent`'s public interface.
...
```bash
git add app/ zero-database/
git commit -m "feat(app): wire DefaultSyncComponent via ApplicationComponent → DatabaseComponent interfaces"
```
