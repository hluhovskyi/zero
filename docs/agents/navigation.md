...
`AttachWithView()` has two overloads — a non-retaining one on a component **instance** (`DisposableEffect`) and a **retaining** one on a `() -> Component` / Dagger `Buildable` that holds the built component in a config-change-surviving `viewModel(key = tag)`. See [di-migration.md](di-migration.md#attachwithview-on----component) for the mechanics; the trap is which one to pick:

- **Never use the retaining overload for a component that captures composition-scoped, Activity-lifecycle-bound objects** (`rememberNavController`, `rememberModalBottomSheetState`) — on a config change the retained component hands the new Activity the *old* NavController, and `NavHost.setGraph` throws `IllegalStateException(... NavController has been destroyed ...)`. Build per composition (`remember { builder.build() }`) and attach via the non-retaining instance overload (`rememberNavController` is already saveable). (ZERO-2 / PR #256, #277.)
