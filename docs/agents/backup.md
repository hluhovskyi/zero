...
- **A spec that pairs an OAuth *refresh-token* flow with "public client / no backend / no client secret" is self-contradictory — flag it at planning time.** Minting a refresh token needs a confidential client (server secret) or a PKCE browser flow; a serverless public client can't. `GetGoogleIdOption` / Credential Manager only *authenticate* (ID token) — they never return a Drive grant. This was missed in the Phase 2 spec; only the real-Drive smoke test caught it (`getAccessToken()` → `AuthExpired`).
- **Play-services-backed auth is not unit-testable — it's manual-smoke-test gated.** Prefer integration tests against `MockWebServer` with real OAuth error shapes (`invalid_grant`, `unauthorized_client`) over fakes that accept anything (which is why the bug above slipped through).

## Restore entry point

**Restore navigates to Import (`Destinations.Import`) and the user taps Google Drive — it does not pre-select.** `ImportUseCase` is `@ImportScope` (built fresh per navigation), so `perform(SelectSource)` on a shared instance never reaches the rendered screen. Drive's `requiresFile = false` → all-new fast path → `RestoreSuccess`; the full round-trip needs interactive OAuth, so it isn't headlessly verifiable.
